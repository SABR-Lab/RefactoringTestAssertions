...
public class AmbariErrorHandlerTest extends org.easymock.EasyMockSupport {
    ...
    public void testHandleInternalServerError() throws java.io.IOException {
        ...
        org.powermock.api.mockito.PowerMockito.mockStatic(org.eclipse.jetty.server.HttpConnection.class, java.util.UUID.class, org.slf4j.LoggerFactory.class);
        org.mockito.Mockito.when(org.eclipse.jetty.server.HttpConnection.getCurrentConnection()).thenReturn(httpConnection);
        org.mockito.Mockito.when(java.util.UUID.randomUUID()).thenReturn(requestId);
        org.mockito.Mockito.when(org.slf4j.LoggerFactory.getLogger(org.apache.ambari.server.api.AmbariErrorHandler.class)).thenReturn(logger);
        ...
        logger.error(org.easymock.EasyMock.capture(captureLogMessage), org.easymock.EasyMock.eq(th));
        org.easymock.EasyMock.expectLastCall();
        org.easymock.EasyMock.expect(httpConnection.getHttpChannel()).andReturn(httpChannel);
        org.easymock.EasyMock.expect(httpChannel.getRequest()).andReturn(request);
        org.easymock.EasyMock.expect(httpChannel.getResponse()).andReturn(response).times(2);
        org.easymock.EasyMock.expect(response.getStatus()).andReturn(org.apache.http.HttpStatus.SC_INTERNAL_SERVER_ERROR);
        final java.lang.String requestUri = "/path/to/target";
        org.easymock.EasyMock.expect(httpServletRequest.getRequestURI()).andReturn(requestUri);
        org.easymock.EasyMock.expect(httpServletRequest.getAttribute(javax.servlet.RequestDispatcher.ERROR_EXCEPTION)).andReturn(th);
        final java.io.StringWriter writer = new java.io.StringWriter();
        org.easymock.EasyMock.expect(httpServletResponse.getWriter()).andReturn(new java.io.PrintWriter(writer));
    ...
    public void testHandleGeneralError() throws java.lang.Exception {
        org.powermock.api.mockito.PowerMockito.mockStatic(org.eclipse.jetty.server.HttpConnection.class);
        org.mockito.Mockito.when(org.eclipse.jetty.server.HttpConnection.getCurrentConnection()).thenReturn(httpConnection);
        org.easymock.EasyMock.expect(httpConnection.getHttpChannel()).andReturn(httpChannel);
        org.easymock.EasyMock.expect(httpChannel.getRequest()).andReturn(request);
        org.easymock.EasyMock.expect(httpChannel.getResponse()).andReturn(response).anyTimes();
        org.easymock.EasyMock.expect(response.getStatus()).andReturn(org.apache.http.HttpStatus.SC_BAD_REQUEST);
        final java.io.StringWriter writer = new java.io.StringWriter();
        org.easymock.EasyMock.expect(httpServletResponse.getWriter()).andReturn(new java.io.PrintWriter(writer));
        org.easymock.EasyMock.expect(propertiesProvider.get()).andReturn(null).anyTimes();
        replayAll();
