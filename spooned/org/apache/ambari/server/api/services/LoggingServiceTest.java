...
public class LoggingServiceTest {
    ...
    private void testGetSearchEngineWhenLogSearchNotRunning(org.springframework.security.core.Authentication authentication, boolean shouldBeAuthorized) throws java.lang.Exception {
        ...
        if (shouldBeAuthorized) {
            org.easymock.EasyMock.expect(uriInfoMock.getQueryParameters()).andReturn(new javax.ws.rs.core.MultivaluedHashMap()).atLeastOnce();
            org.easymock.EasyMock.expect(helperFactoryMock.getHelper(controllerMock, expectedClusterName)).andReturn(null).atLeastOnce();
        }
        org.easymock.EasyMock.expect(controllerFactoryMock.getController()).andReturn(controllerMock).atLeastOnce();
        org.easymock.EasyMock.expect(controllerMock.getClusters()).andReturn(clustersMock).once();
        org.easymock.EasyMock.expect(clustersMock.getCluster(expectedClusterName)).andReturn(clusterMock).once();
        org.easymock.EasyMock.expect(clusterMock.getResourceId()).andReturn(4L).once();
