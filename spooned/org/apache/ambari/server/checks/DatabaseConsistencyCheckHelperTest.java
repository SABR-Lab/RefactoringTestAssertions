...
public class DatabaseConsistencyCheckHelperTest {
    ...
    public void testCheckForConfigsSelectedMoreThanOnce() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement);
        org.easymock.EasyMock.expect(mockStatement.executeQuery((("select c.cluster_name, cc.type_name from clusterconfig cc " + "join clusters c on cc.cluster_id=c.cluster_id ") + "group by c.cluster_name, cc.type_name ") + "having sum(selected) > 1")).andReturn(mockResultSet);
    ...
    public void testCheckForHostsWithoutState() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("select host_name from hosts where host_id not in (select host_id from hoststate)")).andReturn(mockResultSet);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setConnection(mockConnection);
        easyMockSupport.replayAll();
    ...
    public void testCheckHostComponentStatesCountEqualsHostComponentsDesiredStates() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("select count(*) from hostcomponentstate")).andReturn(mockResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("select count(*) from hostcomponentdesiredstate")).andReturn(mockResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery(("select count(*) FROM hostcomponentstate hcs " + "JOIN hostcomponentdesiredstate hcds ON hcs.service_name=hcds.service_name AND ") + "hcs.component_name=hcds.component_name AND hcs.host_id=hcds.host_id")).andReturn(mockResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("select component_name, host_id from hostcomponentstate group by component_name, host_id having count(component_name) > 1")).andReturn(mockResultSet);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setConnection(mockConnection);
        easyMockSupport.replayAll();
    ...
    public void testCheckServiceConfigs() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(mockHDFSServiceInfo.getConfigTypeAttributes()).andReturn(configAttributes);
        org.easymock.EasyMock.expect(mockAmbariMetainfo.getServices("HDP", "2.2")).andReturn(services);
        org.easymock.EasyMock.expect(serviceConfigResultSet.next()).andReturn(true).times(2);
        org.easymock.EasyMock.expect(serviceConfigResultSet.getString("service_name")).andReturn("HDFS").andReturn("HBASE");
        org.easymock.EasyMock.expect(serviceConfigResultSet.getString("type_name")).andReturn("core-site").andReturn("hbase-env");
        org.easymock.EasyMock.expect(stackResultSet.next()).andReturn(true);
        org.easymock.EasyMock.expect(stackResultSet.getString("stack_name")).andReturn("HDP");
        org.easymock.EasyMock.expect(stackResultSet.getString("stack_version")).andReturn("2.2");
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement);
        org.easymock.EasyMock.expect(mockStatement.executeQuery(("select c.cluster_name, service_name from clusterservices cs " + "join clusters c on cs.cluster_id=c.cluster_id ") + "where service_name not in (select service_name from serviceconfig sc where sc.cluster_id=cs.cluster_id and sc.service_name=cs.service_name and sc.group_id is null)")).andReturn(mockResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery(("select c.cluster_name, sc.service_name, sc.version from serviceconfig sc " + "join clusters c on sc.cluster_id=c.cluster_id ") + "where service_config_id not in (select service_config_id from serviceconfigmapping) and group_id is null")).andReturn(mockResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("select c.cluster_name, s.stack_name, s.stack_version from clusters c " + "join stack s on c.desired_stack_id = s.stack_id")).andReturn(stackResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery(((((("select c.cluster_name, cs.service_name, cc.type_name, sc.version from clusterservices cs " + "join serviceconfig sc on cs.service_name=sc.service_name and cs.cluster_id=sc.cluster_id ") + "join serviceconfigmapping scm on sc.service_config_id=scm.service_config_id ") + "join clusterconfig cc on scm.config_id=cc.config_id and sc.cluster_id=cc.cluster_id ") + "join clusters c on cc.cluster_id=c.cluster_id and sc.stack_id=c.desired_stack_id ") + "where sc.group_id is null and sc.service_config_id=(select max(service_config_id) from serviceconfig sc2 where sc2.service_name=sc.service_name and sc2.cluster_id=sc.cluster_id) ") + "group by c.cluster_name, cs.service_name, cc.type_name, sc.version")).andReturn(serviceConfigResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery((((((("select c.cluster_name, cs.service_name, cc.type_name from clusterservices cs " + "join serviceconfig sc on cs.service_name=sc.service_name and cs.cluster_id=sc.cluster_id ") + "join serviceconfigmapping scm on sc.service_config_id=scm.service_config_id ") + "join clusterconfig cc on scm.config_id=cc.config_id and cc.cluster_id=sc.cluster_id ") + "join clusters c on cc.cluster_id=c.cluster_id ") + "where sc.group_id is null and sc.service_config_id = (select max(service_config_id) from serviceconfig sc2 where sc2.service_name=sc.service_name and sc2.cluster_id=sc.cluster_id) ") + "group by c.cluster_name, cs.service_name, cc.type_name ") + "having sum(cc.selected) < 1")).andReturn(mockResultSet);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setConnection(mockConnection);
        easyMockSupport.replayAll();
    ...
    private void setupMocksForTestSchemaName(java.lang.String configuredSchema, java.lang.String searchPath, java.util.List<java.lang.String> schemas, java.util.List<java.lang.String> schemasWithAmbariTables) throws java.lang.Exception {
        final org.apache.ambari.server.configuration.Configuration config = org.easymock.EasyMock.createNiceMock(org.apache.ambari.server.configuration.Configuration.class);
        final org.apache.ambari.server.state.stack.OsFamily osFamily = org.easymock.EasyMock.createNiceMock(org.apache.ambari.server.state.stack.OsFamily.class);
        final java.sql.Connection connection = org.easymock.EasyMock.createNiceMock(java.sql.Connection.class);
        final org.apache.ambari.server.orm.DBAccessor dbAccessor = org.easymock.EasyMock.createStrictMock(org.apache.ambari.server.orm.DBAccessor.class);
        final java.sql.Statement searchPathStatement = org.easymock.EasyMock.createStrictMock(java.sql.Statement.class);
        final java.sql.Statement getTablesStatement = org.easymock.EasyMock.createStrictMock(java.sql.Statement.class);
        final java.sql.DatabaseMetaData dbMetaData = org.easymock.EasyMock.createStrictMock(java.sql.DatabaseMetaData.class);
        final com.google.inject.Injector mockInjector = com.google.inject.Guice.createInjector(new com.google.inject.AbstractModule() {
            @java.lang.Override
            protected void configure() {
        ...
        org.easymock.EasyMock.expect(config.getDatabaseSchema()).andReturn(configuredSchema).anyTimes();
        org.easymock.EasyMock.expect(config.getDatabaseType()).andReturn(org.apache.ambari.server.configuration.Configuration.DatabaseType.POSTGRES);
        org.easymock.EasyMock.expect(dbAccessor.getConnection()).andReturn(connection);
        org.easymock.EasyMock.expect(connection.getMetaData()).andReturn(dbMetaData);
        org.easymock.EasyMock.expect(connection.createStatement()).andReturn(searchPathStatement);
        org.easymock.EasyMock.expect(connection.createStatement()).andReturn(getTablesStatement);
        org.easymock.EasyMock.expect(dbMetaData.getSchemas()).andReturn(resultSet("TABLE_SCHEM", schemas));
        org.easymock.EasyMock.expect(searchPathStatement.executeQuery(org.easymock.EasyMock.anyString())).andReturn(resultSet("search_path", com.google.common.collect.Lists.newArrayList(searchPath)));
        org.easymock.EasyMock.expect(getTablesStatement.executeQuery(org.easymock.EasyMock.anyString())).andReturn(resultSet("table_schema", schemasWithAmbariTables));
        org.easymock.EasyMock.replay(config, connection, dbAccessor, dbMetaData, getTablesStatement, osFamily, searchPathStatement);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
    ...
    private java.sql.ResultSet resultSet(final java.lang.String columnName, final java.util.List<? extends java.lang.Object> columnData) throws java.sql.SQLException {
        if (null == columnData) {
            ...
            java.sql.ResultSet rs = org.easymock.EasyMock.createNiceMock(java.sql.ResultSet.class);
            if (!columnData.isEmpty()) {
                org.easymock.EasyMock.expect(rs.next()).andReturn(true).times(columnData.size());
            }
            org.easymock.EasyMock.expect(rs.next()).andReturn(false);
            for (java.lang.Object item : columnData) {
                org.easymock.EasyMock.expect(rs.getObject(columnName)).andReturn(item);
            }
    ...
    public void testCheckServiceConfigs_missingServiceConfigGeneratesWarning() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(mockHDFSServiceInfo.getConfigTypeAttributes()).andReturn(configAttributes);
        org.easymock.EasyMock.expect(mockAmbariMetainfo.getServices("HDP", "2.2")).andReturn(services);
        org.easymock.EasyMock.expect(clusterServicesResultSet.next()).andReturn(true);
        org.easymock.EasyMock.expect(clusterServicesResultSet.getString("service_name")).andReturn("OPENSOFT R");
        org.easymock.EasyMock.expect(clusterServicesResultSet.getString("cluster_name")).andReturn("My Cluster");
        org.easymock.EasyMock.expect(serviceConfigResultSet.next()).andReturn(true);
        org.easymock.EasyMock.expect(serviceConfigResultSet.getString("service_name")).andReturn("HDFS");
        org.easymock.EasyMock.expect(serviceConfigResultSet.getString("type_name")).andReturn("core-site");
        org.easymock.EasyMock.expect(stackResultSet.next()).andReturn(true);
        org.easymock.EasyMock.expect(stackResultSet.getString("stack_name")).andReturn("HDP");
        org.easymock.EasyMock.expect(stackResultSet.getString("stack_version")).andReturn("2.2");
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement);
    ...
    public void testCheckForLargeTables() throws java.lang.Exception {
        ...
        final com.google.inject.Injector mockInjector = createInjectorWithAmbariMetaInfo(mockAmbariMetainfo, mockDBDbAccessor);
        org.easymock.EasyMock.expect(hostRoleCommandResultSet.next()).andReturn(true).once();
        org.easymock.EasyMock.expect(executionCommandResultSet.next()).andReturn(true).once();
        org.easymock.EasyMock.expect(stageResultSet.next()).andReturn(true).once();
        org.easymock.EasyMock.expect(requestResultSet.next()).andReturn(true).once();
        org.easymock.EasyMock.expect(alertHistoryResultSet.next()).andReturn(true).once();
        org.easymock.EasyMock.expect(hostRoleCommandResultSet.getLong(1)).andReturn(2345L).atLeastOnce();
        org.easymock.EasyMock.expect(executionCommandResultSet.getLong(1)).andReturn(12345L).atLeastOnce();
        org.easymock.EasyMock.expect(stageResultSet.getLong(1)).andReturn(2321L).atLeastOnce();
        org.easymock.EasyMock.expect(requestResultSet.getLong(1)).andReturn(1111L).atLeastOnce();
        org.easymock.EasyMock.expect(alertHistoryResultSet.getLong(1)).andReturn(2223L).atLeastOnce();
        org.easymock.EasyMock.expect(mockDBDbAccessor.getConnection()).andReturn(mockConnection);
        org.easymock.EasyMock.expect(mockDBDbAccessor.getDbType()).andReturn(org.apache.ambari.server.orm.DBAccessor.DbType.MYSQL).anyTimes();
        org.easymock.EasyMock.expect(mockDBDbAccessor.getDbSchema()).andReturn("test_schema");
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement).anyTimes();
        org.easymock.EasyMock.expect(mockStatement.executeQuery("SELECT (data_length + index_length) \"Table Size\" " + "FROM information_schema.TABLES WHERE table_schema = \"test_schema\" AND table_name =\"host_role_command\"")).andReturn(hostRoleCommandResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("SELECT (data_length + index_length) \"Table Size\" " + "FROM information_schema.TABLES WHERE table_schema = \"test_schema\" AND table_name =\"execution_command\"")).andReturn(executionCommandResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("SELECT (data_length + index_length) \"Table Size\" " + "FROM information_schema.TABLES WHERE table_schema = \"test_schema\" AND table_name =\"stage\"")).andReturn(stageResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("SELECT (data_length + index_length) \"Table Size\" " + "FROM information_schema.TABLES WHERE table_schema = \"test_schema\" AND table_name =\"request\"")).andReturn(requestResultSet);
        org.easymock.EasyMock.expect(mockStatement.executeQuery("SELECT (data_length + index_length) \"Table Size\" " + "FROM information_schema.TABLES WHERE table_schema = \"test_schema\" AND table_name =\"alert_history\"")).andReturn(alertHistoryResultSet);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        easyMockSupport.replayAll();
        mockAmbariMetainfo.init();
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.resetCheckResult();
    ...
    public void testConfigGroupHostMappings() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(mockClusters.getClusters()).andReturn(clusters).anyTimes();
        ...
        org.easymock.EasyMock.expect(cluster.getConfigGroups()).andReturn(configGroupMap).anyTimes();
        ...
        hosts.put("h1", h1);
        org.easymock.EasyMock.expect(mockClusters.getHostsForCluster("c1")).andReturn(hosts);
        java.util.Map<java.lang.Long, org.apache.ambari.server.state.Host> cgHosts = new java.util.HashMap<>();
        cgHosts.put(1L, h1);
        cgHosts.put(2L, h2);
        org.easymock.EasyMock.expect(cg1.getHosts()).andReturn(cgHosts);
        org.easymock.EasyMock.expect(h1.getHostName()).andReturn("h1").anyTimes();
        org.easymock.EasyMock.expect(h2.getHostName()).andReturn("h2").anyTimes();
        org.easymock.EasyMock.expect(h1.getHostId()).andReturn(1L).anyTimes();
        org.easymock.EasyMock.expect(h2.getHostId()).andReturn(2L).anyTimes();
        org.easymock.EasyMock.expect(cg1.getId()).andReturn(1L).anyTimes();
        org.easymock.EasyMock.expect(cg2.getId()).andReturn(2L).anyTimes();
        org.easymock.EasyMock.expect(cg1.getName()).andReturn("cg1").anyTimes();
        org.easymock.EasyMock.expect(cg2.getName()).andReturn("cg2").anyTimes();
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        easyMockSupport.replayAll();
    ...
    public void testConfigGroupForDeletedServices() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(cluster.getConfigGroups()).andStubReturn(configGroupMap);
        org.easymock.EasyMock.expect(cg1.getName()).andReturn("cg1").anyTimes();
        org.easymock.EasyMock.expect(cg1.getId()).andReturn(1L).anyTimes();
        org.easymock.EasyMock.expect(cg1.getServiceName()).andReturn("YARN").anyTimes();
        org.easymock.EasyMock.expect(cg2.getServiceName()).andReturn("HDFS").anyTimes();
        org.easymock.EasyMock.expect(cgWithoutServiceName.getName()).andReturn("cg3").anyTimes();
        org.easymock.EasyMock.expect(cgWithoutServiceName.getId()).andReturn(3L).anyTimes();
        org.easymock.EasyMock.expect(cgWithoutServiceName.getServiceName()).andReturn(null).anyTimes();
        ...
        org.easymock.EasyMock.expect(cg1.getClusterName()).andReturn("c1");
        org.easymock.EasyMock.expect(mockClusters.getCluster("c1")).andReturn(cluster).anyTimes();
    ...
    public void testCheckForStalealertdefs() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(staleAlertResultSet.next()).andReturn(true).once();
        org.easymock.EasyMock.expect(staleAlertResultSet.getString("definition_name")).andReturn("ALERT-NAME").atLeastOnce();
        org.easymock.EasyMock.expect(staleAlertResultSet.getString("service_name")).andReturn("SERVICE-DELETED").atLeastOnce();
        final java.sql.Connection mockConnection = easyMockSupport.createNiceMock(java.sql.Connection.class);
        final java.sql.Statement mockStatement = easyMockSupport.createNiceMock(java.sql.Statement.class);
        org.easymock.EasyMock.expect(mockDBDbAccessor.getConnection()).andReturn(mockConnection);
        org.easymock.EasyMock.expect(mockDBDbAccessor.getDbType()).andReturn(org.apache.ambari.server.orm.DBAccessor.DbType.MYSQL);
        org.easymock.EasyMock.expect(mockDBDbAccessor.getDbSchema()).andReturn("test_schema");
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement).anyTimes();
        org.easymock.EasyMock.expect(mockStatement.executeQuery("select definition_name, service_name from alert_definition where service_name not in " + "(select service_name from clusterservices) and service_name not in ('AMBARI')")).andReturn(staleAlertResultSet);
        org.easymock.EasyMock.expect(alertDefinition.getDefinitionId()).andReturn(1L);
        org.easymock.EasyMock.expect(alertDefinition.getName()).andReturn("AlertTest");
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        easyMockSupport.replayAll();
    ...
    public void testCollectConfigGroupsWithoutServiceName() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(cluster2.getConfigGroups()).andReturn(new java.util.HashMap<java.lang.Long, org.apache.ambari.server.state.configgroup.ConfigGroup>(0)).anyTimes();
        org.easymock.EasyMock.expect(mockClusters.getClusters()).andReturn(clusters).anyTimes();
        org.easymock.EasyMock.expect(mockClusters.getCluster("c1")).andReturn(cluster1).anyTimes();
        org.easymock.EasyMock.expect(mockClusters.getCluster("c2")).andReturn(cluster2).anyTimes();
        java.util.Map<java.lang.Long, org.apache.ambari.server.state.configgroup.ConfigGroup> configGroupMap = new java.util.HashMap<>();
        org.apache.ambari.server.state.configgroup.ConfigGroup cgWithoutServiceName = easyMockSupport.createNiceMock(org.apache.ambari.server.state.configgroup.ConfigGroup.class);
        ...
        org.easymock.EasyMock.expect(cluster1.getConfigGroups()).andReturn(configGroupMap).anyTimes();
        org.easymock.EasyMock.expect(cgWithoutServiceName.getId()).andReturn(1L).anyTimes();
        org.easymock.EasyMock.expect(cgWithoutServiceName.getClusterName()).andReturn("c1").anyTimes();
        org.easymock.EasyMock.expect(cgWithoutServiceName.getServiceName()).andReturn(null).anyTimes();
        org.easymock.EasyMock.expect(cgWithoutServiceName.getTag()).andReturn("YARN").anyTimes();
        cgWithoutServiceName.setServiceName("YARN");
        org.easymock.EasyMock.expectLastCall();
        org.easymock.EasyMock.expect(cgWithServiceName.getId()).andReturn(2L).anyTimes();
        org.easymock.EasyMock.expect(cgWithServiceName.getClusterName()).andReturn("c1").anyTimes();
        org.easymock.EasyMock.expect(cgWithServiceName.getServiceName()).andReturn("HDFS").anyTimes();
        org.easymock.EasyMock.expect(cgForNonExistentService.getId()).andReturn(3L).anyTimes();
        org.easymock.EasyMock.expect(cgForNonExistentService.getClusterName()).andReturn("c1").anyTimes();
        org.easymock.EasyMock.expect(cgForNonExistentService.getServiceName()).andReturn(null).anyTimes();
        org.easymock.EasyMock.expect(cgForNonExistentService.getTag()).andReturn("NOT_EXISTS").anyTimes();
        ...
        org.easymock.EasyMock.expect(cluster1.getServices()).andReturn(services).anyTimes();
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        easyMockSupport.replayAll();
    ...
    public void testFixConfigsSelectedMoreThanOnce() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(mockConnection.createStatement(java.sql.ResultSet.TYPE_SCROLL_SENSITIVE, java.sql.ResultSet.CONCUR_UPDATABLE)).andReturn(mockStatement);
        org.easymock.EasyMock.expect(mockStatement.executeQuery((("select c.cluster_name, cc.type_name from clusterconfig cc " + "join clusters c on cc.cluster_id=c.cluster_id ") + "group by c.cluster_name, cc.type_name ") + "having sum(cc.selected) > 1")).andReturn(mockResultSet);
        org.easymock.EasyMock.expect(mockResultSet.next()).andReturn(true).once();
        org.easymock.EasyMock.expect(mockResultSet.getString("cluster_name")).andReturn("123").once();
        org.easymock.EasyMock.expect(mockResultSet.getString("type_name")).andReturn("type1").once();
        org.easymock.EasyMock.expect(mockResultSet.next()).andReturn(false).once();
        ...
        org.easymock.EasyMock.expect(mockClusters.getCluster("123")).andReturn(clusterMock);
        org.easymock.EasyMock.expect(clusterMock.getClusterId()).andReturn(123L).once();
        ...
        org.easymock.EasyMock.expect(clusterConfigEntity1.getType()).andReturn("type1").anyTimes();
        org.easymock.EasyMock.expect(clusterConfigEntity1.getSelectedTimestamp()).andReturn(123L);
        clusterConfigEntity1.setSelected(false);
        org.easymock.EasyMock.expectLastCall().once();
        org.easymock.EasyMock.expect(clusterConfigEntity2.getType()).andReturn("type1").anyTimes();
        org.easymock.EasyMock.expect(clusterConfigEntity2.getSelectedTimestamp()).andReturn(321L);
        ...
        org.easymock.EasyMock.expect(mockEntityManager.createNamedQuery(org.easymock.EasyMock.anyString(), org.easymock.EasyMock.anyObject(java.lang.Class.class))).andReturn(queryMock).anyTimes();
        org.easymock.EasyMock.expect(queryMock.setParameter(org.easymock.EasyMock.anyString(), org.easymock.EasyMock.anyString())).andReturn(queryMock).once();
        org.easymock.EasyMock.expect(queryMock.setParameter(org.easymock.EasyMock.anyString(), org.easymock.EasyMock.anyLong())).andReturn(queryMock).once();
        org.easymock.EasyMock.expect(queryMock.getResultList()).andReturn(java.util.Arrays.asList(clusterConfigEntity1, clusterConfigEntity2)).once();
        org.easymock.EasyMock.expect(clusterDAO.merge(org.easymock.EasyMock.anyObject(org.apache.ambari.server.orm.entities.ClusterConfigEntity.class), org.easymock.EasyMock.anyBoolean())).andReturn(null).times(3);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setInjector(mockInjector);
        org.apache.ambari.server.checks.DatabaseConsistencyCheckHelper.setConnection(mockConnection);
        easyMockSupport.replayAll();
