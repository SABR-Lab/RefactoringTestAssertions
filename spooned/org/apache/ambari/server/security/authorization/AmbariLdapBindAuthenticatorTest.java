...
public class AmbariLdapBindAuthenticatorTest extends org.easymock.EasyMockSupport {
    ...
    public void testAuthenticateBadPassword() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(ldapCtxSource.getBaseLdapName()).andReturn(basePath).atLeastOnce();
        org.easymock.EasyMock.expect(ldapCtxSource.getContext(ldapUserDNString, "password")).andThrow(new org.springframework.ldap.AuthenticationException(null)).once();
        ...
        org.easymock.EasyMock.expect(searchedUserContext.getDn()).andReturn(ldapUserRelativeDN).atLeastOnce();
        ...
        org.easymock.EasyMock.expect(userSearch.searchForUser(org.easymock.EasyMock.anyString())).andReturn(searchedUserContext).once();
    ...
    private void testAuthenticate(java.lang.String ambariUsername, java.lang.String ldapUsername, boolean forceUsernameToLower) throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(adminGroups.hasMore()).andReturn(false).atLeastOnce();
        adminGroups.close();
        ...
        org.easymock.EasyMock.expect(boundUserContext.search(org.easymock.EasyMock.eq("ou=groups"), org.easymock.EasyMock.eq(("(&(member=" + ldapUserDNString) + ")(objectclass=group)(|(cn=Ambari Administrators)))"), org.easymock.EasyMock.anyObject(javax.naming.directory.SearchControls.class))).andReturn(adminGroups).atLeastOnce();
        boundUserContext.close();
        org.easymock.EasyMock.expectLastCall().atLeastOnce();
        org.springframework.ldap.core.support.LdapContextSource ldapCtxSource = createMock(org.springframework.ldap.core.support.LdapContextSource.class);
        org.easymock.EasyMock.expect(ldapCtxSource.getBaseLdapName()).andReturn(basePath).atLeastOnce();
        org.easymock.EasyMock.expect(ldapCtxSource.getContext(ldapUserDNString, "password")).andReturn(boundUserContext).once();
        org.easymock.EasyMock.expect(ldapCtxSource.getReadOnlyContext()).andReturn(boundUserContext).once();
        javax.naming.directory.Attributes searchedAttributes = new javax.naming.directory.BasicAttributes("uid", ldapUsername);
        org.springframework.ldap.core.DirContextOperations searchedUserContext = createMock(org.springframework.ldap.core.DirContextOperations.class);
        org.easymock.EasyMock.expect(searchedUserContext.getDn()).andReturn(ldapUserRelativeDN).atLeastOnce();
        org.easymock.EasyMock.expect(searchedUserContext.getAttributes()).andReturn(searchedAttributes).atLeastOnce();
        org.springframework.security.ldap.search.FilterBasedLdapUserSearch userSearch = createMock(org.springframework.security.ldap.search.FilterBasedLdapUserSearch.class);
        org.easymock.EasyMock.expect(userSearch.searchForUser(ambariUsername)).andReturn(searchedUserContext).once();
        ...
        if ((!org.apache.commons.lang.StringUtils.isEmpty(ldapUsername)) && (!ambariUsername.equals(ldapUsername))) {
            servletRequestAttributes.setAttribute(org.easymock.EasyMock.eq(ambariUsername), org.easymock.EasyMock.eq(forceUsernameToLower ? ldapUsername.toLowerCase() : ldapUsername), org.easymock.EasyMock.eq(org.springframework.web.context.request.RequestAttributes.SCOPE_SESSION));
            org.easymock.EasyMock.expectLastCall().once();
            servletRequestAttributes.setAttribute(org.easymock.EasyMock.eq(forceUsernameToLower ? ldapUsername.toLowerCase() : ldapUsername), org.easymock.EasyMock.eq(ambariUsername), org.easymock.EasyMock.eq(org.springframework.web.context.request.RequestAttributes.SCOPE_SESSION));
            org.easymock.EasyMock.expectLastCall().once();
        }
    ...
    private void setupDatabaseConfigurationExpectations(boolean expectedDatabaseConfigCall, boolean forceUsernameToLowerCase) {
        ...
        if (expectedDatabaseConfigCall) {
            org.easymock.EasyMock.expect(ldapConfiguration.getLdapServerProperties()).andReturn(ldapServerProperties).anyTimes();
        }
    }
