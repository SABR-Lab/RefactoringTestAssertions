...
public class ConfigureAmbariIdentitiesServerActionTest extends org.easymock.EasyMockSupport {
    ...
    private void installAmbariServerIdentity(org.apache.ambari.server.serveraction.ActionLog actionLog, boolean ambariServerHasAgent) throws java.lang.Exception {
        ...
        if (ambariServerHasAgent) {
            ...
            org.easymock.EasyMock.expect(hostEntity.getHostId()).andReturn(1L).once();
            org.easymock.EasyMock.expect(hostDAO.findById(1L)).andReturn(hostEntity).once();
        } else {
            hostEntity = null;
        }
        org.easymock.EasyMock.expect(hostDAO.findByName(org.apache.ambari.server.utils.StageUtils.getHostName())).andReturn(hostEntity).once();
        ...
        result.created = true;
        result.kkp = kkp;
        org.easymock.EasyMock.expect(kerberosKeytabPrincipalDAO.findOrCreate(org.easymock.EasyMock.anyObject(), org.easymock.EasyMock.eq(hostEntity), org.easymock.EasyMock.anyObject(), org.easymock.EasyMock.anyObject())).andReturn(result).once();
        org.easymock.EasyMock.expect(kerberosKeytabPrincipalDAO.merge(kkp)).andReturn(createNiceMock(org.apache.ambari.server.orm.entities.KerberosKeytabPrincipalEntity.class)).once();
    ...
    private void configureJAAS(org.apache.ambari.server.serveraction.ActionLog actionLog) throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(action.getJAASConfFilePath()).andReturn(jaasConfFile.getAbsolutePath());
        replayAll();
        injector.injectMembers(action);
