...
public class FinalizeKerberosServerActionTest extends org.easymock.EasyMockSupport {
    ...
    public void executeMITKDCOption() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(kerberosHelper.getKDCAdministratorCredentials(clusterName)).andReturn(principleKeyCredential).anyTimes();
        replayAll();
    ...
    private org.apache.ambari.server.agent.ExecutionCommand createMockExecutionCommand(java.lang.String clusterId, java.lang.String clusterName, java.util.Map<java.lang.String, java.lang.String> commandParams) {
        ...
        org.easymock.EasyMock.expect(executionCommand.getClusterId()).andReturn(clusterId).anyTimes();
        org.easymock.EasyMock.expect(executionCommand.getClusterName()).andReturn(clusterName).anyTimes();
        org.easymock.EasyMock.expect(executionCommand.getCommandParams()).andReturn(commandParams).anyTimes();
        org.easymock.EasyMock.expect(executionCommand.getRoleCommand()).andReturn(org.apache.ambari.server.RoleCommand.EXECUTE).anyTimes();
        org.easymock.EasyMock.expect(executionCommand.getRole()).andReturn(org.apache.ambari.server.Role.AMBARI_SERVER_ACTION.name()).anyTimes();
        org.easymock.EasyMock.expect(executionCommand.getServiceName()).andReturn(org.apache.ambari.server.controller.RootComponent.AMBARI_SERVER.name()).anyTimes();
        org.easymock.EasyMock.expect(executionCommand.getTaskId()).andReturn(3L).anyTimes();
    ...
    private org.apache.ambari.server.actionmanager.HostRoleCommand createMockHostRoleCommand() {
        ...
        org.easymock.EasyMock.expect(hostRoleCommand.getRequestId()).andReturn(1L).anyTimes();
        org.easymock.EasyMock.expect(hostRoleCommand.getStageId()).andReturn(2L).anyTimes();
        org.easymock.EasyMock.expect(hostRoleCommand.getTaskId()).andReturn(3L).anyTimes();
    ...
    private com.google.inject.Injector setup(java.lang.String clusterName) throws org.apache.ambari.server.AmbariException {
        ...
        org.easymock.EasyMock.expect(serviceComponentHost.getServiceName()).andReturn("SERVICE1").anyTimes();
        org.easymock.EasyMock.expect(serviceComponentHost.getServiceComponentName()).andReturn("COMPONENT1A").anyTimes();
        org.easymock.EasyMock.expect(serviceComponentHost.getHostName()).andReturn("host1").anyTimes();
        ...
        org.easymock.EasyMock.expect(cluster.getServiceComponentHosts("host1")).andReturn(serviceComponentHosts).anyTimes();
        final org.apache.ambari.server.state.Clusters clusters = createMock(org.apache.ambari.server.state.Clusters.class);
        org.easymock.EasyMock.expect(clusters.getHostsForCluster(clusterName)).andReturn(clusterHostMap).anyTimes();
        ...
        org.easymock.EasyMock.expect(topologyHolder.getCurrentData()).andReturn(event).once();
        org.easymock.EasyMock.expect(topologyHolder.updateData(event)).andReturn(org.mockito.Matchers.anyBoolean()).once();
        return com.google.inject.Guice.createInjector(new com.google.inject.AbstractModule() {
            ...
            protected void configure() {
                ...
                bind(javax.persistence.EntityManager.class).toProvider(((com.google.inject.Provider<? extends javax.persistence.EntityManager>) (org.easymock.EasyMock.createNiceMock(com.google.inject.Provider.class))));
            }
        });
    }
}
