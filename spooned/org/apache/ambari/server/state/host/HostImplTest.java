...
public class HostImplTest extends org.easymock.EasyMockSupport {
    ...
    public void testGetHostAttributes() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(hostEntity.getHostAttributes()).andReturn("{\"foo\": \"aaa\", \"bar\":\"bbb\"}").anyTimes();
        org.easymock.EasyMock.expect(hostEntity.getHostId()).andReturn(1L).anyTimes();
        org.easymock.EasyMock.expect(hostEntity.getHostName()).andReturn("host1").anyTimes();
        org.easymock.EasyMock.expect(hostEntity.getHostStateEntity()).andReturn(hostStateEntity).anyTimes();
        org.easymock.EasyMock.expect(hostDAO.findById(1L)).andReturn(hostEntity).atLeastOnce();
        replayAll();
    ...
    public void testGetHealthStatus() throws java.lang.Exception {
        ...
        org.easymock.EasyMock.expect(hostEntity.getHostId()).andReturn(1L).anyTimes();
        org.easymock.EasyMock.expect(hostEntity.getHostStateEntity()).andReturn(hostStateEntity).anyTimes();
        org.easymock.EasyMock.expect(hostDAO.findById(1L)).andReturn(hostEntity).anyTimes();
        org.easymock.EasyMock.expect(hostStateDAO.findByHostId(1L)).andReturn(hostStateEntity).atLeastOnce();
        replayAll();
        org.apache.ambari.server.state.host.HostImpl host = new org.apache.ambari.server.state.host.HostImpl(hostEntity, gson, hostDAO, hostStateDAO);
