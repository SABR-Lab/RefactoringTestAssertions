...
public class DownloadService extends org.apache.ambari.view.commons.hdfs.HdfsService {
    ...
    javax.ws.rs.core.UriInfo ui) {
        ...
        try {
            ...
            if (checkperm) {
                ...
                return javax.ws.rs.core.Response.ok(jsonObject).header("Content-Type", javax.ws.rs.core.MediaType.APPLICATION_JSON).build();
            }
            javax.ws.rs.core.Response.ResponseBuilder result = javax.ws.rs.core.Response.ok(fs);
            if (download) {
                result.header("Content-Disposition", ("attachment; filename=\"" + java.net.URLEncoder.encode(status.getPath().getName(), "UTF-8")) + "\"").type(javax.ws.rs.core.MediaType.APPLICATION_OCTET_STREAM);
            } else {
    ...
    public javax.ws.rs.core.Response concat(final org.apache.ambari.view.filebrowser.DownloadService.DownloadRequest request) {
        ...
        try {
            ...
            if (request.download) {
                response.header("Content-Disposition", "attachment; filename=\"concatResult.txt\"").type(javax.ws.rs.core.MediaType.APPLICATION_OCTET_STREAM);
            } else {
                response.header("Content-Disposition", "filename=\"concatResult.txt\"").type(javax.ws.rs.core.MediaType.TEXT_PLAIN);
            }
